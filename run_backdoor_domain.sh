...
for model in "${MODELS[@]}"; do
    for agg in "${AGGREGATIONS[@]}"; do
        ...
        if [[ "$model" == mobilenetv2 && ( "$agg" == fedavg || "$agg" == krum ) ]]; then
            continue
        fi

        echo "Running dataset=domain, model=${model}, aggregation=${agg}, krum_m=${KRUM_M}, aug_mult=10, batch_size=32, auto_aug=true"
        "$PYTHON" "$SCRIPT" \
            --dataset domain \
            --model "$model" \
            --aggregation "$agg" \
